...
from cpp_dev.package.store import (
    PackageStore,
    compose_index_path,
    compose_package_path,
    compose_repositories_path,
)
...
def test_store_index(package_store: PackageStore):
...
def test_store_package(package_store: PackageStore):
...
def test_store_repositories(package_store: PackageStore):
    repositories = package_store.get_repositories()
...
def test_compose_repositories_path(test_os: OperatingSystem) -> None:
    assert compose_repositories_path(test_os) == Path(
        "repositories/x86_64-os-version/repositories.json"
    )
