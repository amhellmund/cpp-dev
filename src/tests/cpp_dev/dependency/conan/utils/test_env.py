...
TEST_PACKAGES = [
    ...
    ConanTestPackage(
        ref=ConanPackageReferenceWithSemanticVersion("test/1.0.0@official/cppdev"),
...
def test_create_conan_env(tmp_path: Path, unused_http_port: int) -> None:
    with create_conan_test_env(tmp_path, unused_http_port, TEST_PACKAGES) as conan_env:
        ...
        assert ConanPackageReferenceWithSemanticVersion("test/1.0.0@official/cppdev") in result