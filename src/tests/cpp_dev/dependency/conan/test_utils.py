...
from cpp_dev.dependency.conan.utils import (CONAN_HOME_ENV_VAR,
                                            compose_conan_package_reference,
                                            conan_env, create_conanfile)
...
def test_create_conanfile(tmp_path: Path) -> None:
    package_deps = [
        DependencySpecifier("official/cpd[1.0.0]"),
        DependencySpecifier("custom/other[>2.0.0]"),
        DependencySpecifier("custom/other2[>=3.0.0,<4.0.0]"),
    ]
...
def test_compose_conan_package_reference(ref: str, expected_conan_ref) -> None:
    conan_ref = compose_conan_package_reference(DependencySpecifier(ref))
